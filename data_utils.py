...
def SD2_instance_generator(config):
    ...
    :return: a fjsp instance generated by SD2, with
        ...
        op_pt: the processing time matrix with shape [N, M],
               where op_pt[i,j] is the processing time of the ith operation 第 i 个工件在第 j 个机器上的加工时间，
               on the jth machine or 0 if $O_i$ can not process on $M_j$
        op_per_mch : the average number of compatible machines of each operation 每个工序的平均兼容机器数量
    """
    n_j = config.n_j
    n_m = config.n_m
    ...
    if data_suffix == "nf":
        op_per_mch_max = 1
    elif data_suffix == "mix":  # JSP（非柔性）一个工序只能由一个机器加工，所以 op_per_mch_max = n_m = op_per_mch_min
        op_per_mch_max = n_m
    else:
...
def load_data_from_files(directory):
    ...
    for root, dirs, files in os.walk(directory):
        ...
        for f in files:
            ...
            job_length, op_pt = text_to_matrix(g)  # 每个工件的工序数；操作 i 在机器 j 上的加工时间
